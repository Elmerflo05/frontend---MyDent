...
class HttpClient {
  ...
  private getUserBranchId(): number | null {
  ...
  private appendBranchFilter(endpoint: string): string {
    const branchId = this.getUserBranchId();
    if (!branchId) return endpoint;
    const excluded = ['/auth/', '/catalogs/', '/health-plans', '/medications', '/statistics/global', '/me', '/profile', '/patients'];
    if (excluded.some(e => endpoint.includes(e))) return endpoint;
    if (endpoint.includes('branch_id=')) return endpoint;
    const sep = endpoint.includes('?') ? '&' : '?';
    return `${endpoint}${sep}branch_id=${branchId}`;
  }
  ...
  public async get<T = any>(
    ...
    const filteredEndpoint = this.appendBranchFilter(endpoint);
    return this.request<T>(filteredEndpoint, { ...config, method: 'GET' });
  }
