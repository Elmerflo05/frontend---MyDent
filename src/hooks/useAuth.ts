...
export const useAuth = (): UseAuthReturn => {
  ...
  const handleLogin = async (credentials: LoginCredentials): Promise<boolean> => {
    ...
    if (success) {
      ...
      // ✅ Inicializar datos protegidos después de login exitoso
      if (freshUser && isAuth) {
        ...
        // Inicializar administradores (requiere autenticación)
        try {
          const { initializeAdministrators } = useAdministratorStore.getState();
          await initializeAdministrators();
          logger.info('✅ Administradores inicializados correctamente');
        } catch (error) {
          logger.error('Error al inicializar administradores después del login', error);
        }

        // Cargar catálogos de odontograma (condiciones dentales, posiciones, superficies)
        try {
          const { loadCatalogsFromDB } = useOdontogramConfigStore.getState();
          await loadCatalogsFromDB();
          logger.info('✅ Catálogos de odontograma cargados correctamente');
        } catch (error) {
          logger.error('Error al cargar catálogos de odontograma después del login', error);
        }
