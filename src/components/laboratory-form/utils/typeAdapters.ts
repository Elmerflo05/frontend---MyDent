 ...
 *
 * Proporciona funciones helper para trabajar con datos de tomografía.
 *
 * NOTA: Con la migración a tipos unificados, la función toLegacyTomografiaFormData
 * ya no es necesaria para transformToSubmission (que ahora acepta tipos separados).
 * Se mantiene por compatibilidad con código existente que pueda depender de ella.
 */
...
/**
 * Helper para determinar si hay opciones de tomografía seleccionadas
 */
export function hasTomografiaSelections(tomografia: Tomografia3DFormData): boolean {
  return (
    tomografia.conInforme ||
    tomografia.sinInforme ||
    tomografia.endodoncia ||
    tomografia.fracturaRadicular ||
    tomografia.anatomiaEndodontica ||
    tomografia.localizacionDiente ||
    tomografia.implantes ||
    tomografia.maxilarSuperior ||
    tomografia.viaAerea ||
    tomografia.ortognatica ||
    tomografia.marpe ||
    tomografia.miniImplantes ||
    tomografia.atm ||
    tomografia.macizoFacial
  );
}

/**
 * Tipo de entrega helper
 */
export function getTipoEntrega(tomografia: Tomografia3DFormData): string {
  if (tomografia.conInforme) return 'conInforme';
  if (tomografia.sinInforme) return 'sinInforme';
  return '';
}
